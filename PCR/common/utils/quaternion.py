...
def torch_euler2quat(e, order):
    ...
    rx = torch.stack((torch.cos(x / 2), torch.sin(
        x / 2), torch.zeros_like(x).cuda(), torch.zeros_like(x).cuda()),
                     dim=1)
    ry = torch.stack(
        (torch.cos(y / 2), torch.zeros_like(y).cuda(), torch.sin(
            y / 2), torch.zeros_like(y).cuda()),
        dim=1)
    rz = torch.stack((torch.cos(z / 2), torch.zeros_like(z).cuda(),
                      torch.zeros_like(z).cuda(), torch.sin(z / 2)),
                     dim=1)
...
def torch_quat2mat(pose):
    ...
    R = torch.stack((torch.stack(
        (R11, R12, R13), dim=0), torch.stack(
            (R21, R22, R23), dim=0), torch.stack((R31, R32, R33), dim=0)),
                    dim=0)
...
def torch_quat_transform(pose_7d: torch.Tensor,
                         pc: torch.Tensor,
                         normal: torch.Tensor = None):
    pc_t = torch_quat_rotate(pc, pose_7d) + pose_7d[:, 4:].view(
        -1, 1, 3).repeat(1, pc.shape[1], 1)  # Ps" = R*Ps + t
