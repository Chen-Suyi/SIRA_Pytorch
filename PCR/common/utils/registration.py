...
from common.utils.pointcloud import (
    apply_transform,
...
def compute_relative_rotation_error(gt_rotation: np.ndarray,
                                    est_rotation: np.ndarray):
...
def compute_relative_translation_error(gt_translation: np.ndarray,
                                       est_translation: np.ndarray):
...
def compute_registration_error(gt_transform: np.ndarray,
    ...
    Returns:
        ...
        rte (float): relative translation error.
    """
    gt_rotation, gt_translation = get_rotation_translation_from_transform(
        gt_transform)
...
def compute_rotation_mse_and_mae(gt_rotation: np.ndarray,
                                 est_rotation: np.ndarray):
...
def compute_translation_mse_and_mae(gt_translation: np.ndarray,
                                    est_translation: np.ndarray):
...
def compute_transform_mse_and_mae(gt_transform: np.ndarray,
                                  est_transform: np.ndarray):
    ...
    t_mse, t_mae = compute_translation_mse_and_mae(gt_translation,
                                                   est_translation)
    return r_mse, r_mae, t_mse, t_mae


def compute_registration_rmse(src_points: np.ndarray, gt_transform: np.ndarray,
                              est_transform: np.ndarray):
...
def compute_modified_chamfer_distance(
    ...
    chamfer_distance_p_q = get_nearest_neighbor(aligned_src_points,
                                                raw_points).mean()
    # Q -> P_raw
    ...
    chamfer_distance_q_p = get_nearest_neighbor(ref_points,
                                                aligned_raw_points).mean()
    # sum up
    chamfer_distance = chamfer_distance_p_q + chamfer_distance_q_p
    return chamfer_distance


def compute_correspondence_residual(ref_corr_points, src_corr_points,
                                    transform):
...
def compute_inlier_ratio(ref_corr_points,
                         src_corr_points,
                         transform,
                         positive_radius=0.1):
...
def compute_overlap(ref_points,
                    src_points,
                    transform=None,
                    positive_radius=0.1):
...
def extract_corr_indices_from_feats(
    ...
    Returns:
        ...
        src_corr_indices: (M,)
    """
    ref_nn_indices = get_nearest_neighbor(ref_feats,
                                          src_feats,
                                          return_index=True)[1]
    if mutual or bilateral:
        src_nn_indices = get_nearest_neighbor(src_feats,
                                              ref_feats,
                                              return_index=True)[1]
        ...
        if mutual:
            ...
            ref_corr_indices = np.concatenate([ref_indices, src_nn_indices],
                                              axis=0)
            src_corr_indices = np.concatenate([ref_nn_indices, src_indices],
                                              axis=0)
    else:
...
def evaluate_correspondences(ref_points,
                             src_points,
                             transform,
                             positive_radius=0.1):
    overlap = compute_overlap(ref_points,
                              src_points,
                              transform,
                              positive_radius=positive_radius)
    inlier_ratio = compute_inlier_ratio(ref_points,
                                        src_points,
                                        transform,
                                        positive_radius=positive_radius)
    residual = compute_correspondence_residual(ref_points, src_points,
                                               transform)
...
def evaluate_sparse_correspondences(ref_points, src_points, ref_corr_indices,
                                    src_corr_indices, gt_corr_indices):
