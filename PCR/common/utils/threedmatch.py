...
from common.utils.registration import compute_overlap, compute_registration_error
...
def get_num_fragments(scene_name):
    if scene_name not in _scene_name_to_num_fragments:
        raise ValueError(
            'Unsupported test scene name "{}".'.format(scene_name))
...
def write_log_file(file_name, test_pairs):
    ...
    for test_pair in test_pairs:
        ...
        lines.append('{}\t{}\t{}\n'.format(frag_id0, frag_id1,
                                           test_pair['num_fragments']))
...
def compute_transform_error(transform, covariance, estimated_transform):
    relative_transform = np.matmul(np.linalg.inv(transform),
                                   estimated_transform)
...
def evaluate_registration_one_scene(gt_log_file,
                                    gt_info_file,
                                    result_file,
    ...
    for result_log in result_logs:
        ...
        if gt_indices[frag_id0, frag_id1] != -1:
            ...
            assert gt_infos[gt_index]['test_pair'][0] == frag_id0 and gt_infos[
                gt_index]['test_pair'][1] == frag_id1
...
def calibrate_ground_truth(ref_pcd, src_pcd, transform, voxel_size=0.006):
    ...
    overlap = compute_overlap(ref_points,
                              src_points,
                              transform,
                              positive_radius=voxel_size * 5)

    # compute info
    src_points = apply_transform(src_points, transform)
    nn_distances, nn_indices = get_nearest_neighbor(ref_points,
                                                    src_points,
                                                    return_index=True)
