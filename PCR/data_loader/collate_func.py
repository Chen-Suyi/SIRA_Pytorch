...
from common.utils.func import grid_subsample, radius_search


def precompute_data_stack_mode(points, lengths, num_stages, voxel_size, radius,
                               neighbor_limits):
    assert num_stages == len(neighbor_limits)

    points_list = []  # num_stages List of (Ni, 3)
    lengths_list = []  # num_stages List of (B,)
    neighbors_list = []  # num_stages List of (Ni, k), q in i, s in i
    subsampling_list = []  # num_stages-1 List of (Ni+1, k), q in i+1, s in i
    upsampling_list = []  # num_stages-1 List of (Ni+1, k), q in i, s in i+1

    # grid subsampling
    for i in range(num_stages):
        if i > 0:
            points, lengths = grid_subsample(points,
                                             lengths,
    ...
    for i in range(num_stages):
        ...
        neighbors = radius_search(
            ...
            radius,
            neighbor_limits[i],
        )  # (Ni, k) Ni is the number of points in current stage, k is neighbor_limits
...
def registration_collate_fn_stack_mode(data_dicts,
    ...
    feats = torch.cat(collated_dict.pop('ref_feats') +
                      collated_dict.pop('src_feats'),
                      dim=0)
    points_list = collated_dict.pop('ref_points') + collated_dict.pop(
        'src_points')
...
def calibrate_neighbors_stack_mode(dataset,
    ...
    neighbor_limits = np.sum(cum_sum < (keep_ratio * cum_sum[hist_n - 1, :]),
                             axis=0)

    # print("max_neighbor_limits:\n", max_neighbor_limits)
    # print("cum_sum:\n", np.sum(cum_sum))
