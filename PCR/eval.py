...
from model.module import weighted_procrustes
from common.utils.summary_board import SummaryBoard
from common.utils.open3d import registration_with_ransac_from_correspondences
from common.utils.registration import (
    evaluate_sparse_correspondences,
    evaluate_correspondences,
    compute_registration_error,
)
from common.utils.threedmatch import (
    get_num_fragments,
    get_scene_abbr,
...
def make_parser():
    ...
    parser.add_argument('--method',
                        choices=['lgr', 'ransac', 'svd'],
                        required=True,
                        help='registration method')
    parser.add_argument('--num_corr',
                        type=int,
                        default=None,
                        help='number of correspondences for registration')
...
def eval_one_epoch(args, cfg, logger):
    features_root = osp.join(cfg.model_dir, 'features', args.benchmark)
    ...
    all_scan_pair_meter = SummaryBoard()
    all_scan_pair_meter.register_meter('recall')
    all_scan_pair_meter.register_meter('rre')
    all_scan_pair_meter.register_meter('rte')
    ...
    for scene_root in scene_roots:
        ...
        if benchmark in ['3DMatch', '3DLoMatch']:
            gt_root = osp.join(cfg.data.dataset_root, '3DMatch', 'metadata',
                               'benchmarks', benchmark, scene_name)
        else:
            # ETH
            gt_root = osp.join("/data/codes/RoReg/data/origin_data/", 'ETH',
                               scene_name, "PointCloud")
        ...
        file_names = sorted(
            ...
            key=lambda x:
            [int(i) for i in osp.basename(x).split('.')[0].split('_')],
        )
        for file_name in file_names:
            ref_frame, src_frame = [
                int(x)
                for x in osp.basename(file_name).split('.')[0].split('_')
            ]
            ...
            if args.num_corr is not None and corr_scores.shape[
                    0] > args.num_corr:
            ...
            message = '{}, id0: {}, id1: {}, OV: {:.3f}'.format(
                scene_abbr, ref_frame, src_frame, pcd_overlap)

            # 1. evaluate correspondences
            # 1.1 evaluate coarse correspondences
            coarse_matching_result_dict = evaluate_sparse_correspondences(
                ref_points_c, src_points_c, ref_node_corr_indices,
                src_node_corr_indices, gt_node_corr_indices)

            coarse_precision = coarse_matching_result_dict['precision']

            coarse_matching_meter.update('scene_precision', coarse_precision)
            coarse_matching_meter.update('scene_PMR>0',
                                         float(coarse_precision > 0))
            coarse_matching_meter.update('scene_PMR>=0.1',
                                         float(coarse_precision >= 0.1))
            coarse_matching_meter.update('scene_PMR>=0.3',
                                         float(coarse_precision >= 0.3))
            coarse_matching_meter.update('scene_PMR>=0.5',
                                         float(coarse_precision >= 0.5))
            ...
            message += ', f_RS: {:.3f}'.format(
                fine_matching_result_dict['residual'])
            message += ', f_NU: {}'.format(
                fine_matching_result_dict['num_corr'])
            ...
            if args.method == 'lgr':
                ...
                with torch.no_grad():
                    ...
                    estimated_transform = estimated_transform.detach().cpu(
                    ).numpy()
            else:
                raise ValueError(
                    f'Unsupported registration method: {args.method}.')
            ...
            if gt_indices[ref_frame, src_frame] != -1:
                ...
                error = compute_transform_error(transform, covariance,
                                                estimated_transform)
                message += ', r_RMSE: {:.3f}'.format(np.sqrt(error))
                accepted = error < cfg.eval.acceptance_rmse**2
                registration_meter.update('scene_recall', float(accepted))
                all_scan_pair_meter.update('recall', float(accepted))
                if accepted:
                    rre, rte = compute_registration_error(
                        transform, estimated_transform)
                    registration_meter.update('scene_rre', rre)
                    registration_meter.update('scene_rte', rte)
                    all_scan_pair_meter.update('rre', rre)
                    all_scan_pair_meter.update('rte', rte)
        ...
        est_log = osp.join(cfg.model_dir, 'registration', benchmark,
                           scene_name, 'est.log')
        ...
        scene_fine_matching_result_dict[scene_abbr] = {
            'recall': recall,
    ...
    message += ', PMR>=0.1: {:.3f}'.format(
        coarse_matching_meter.mean('PMR>=0.1'))
    message += ', PMR>=0.3: {:.3f}'.format(
        coarse_matching_meter.mean('PMR>=0.3'))
    message += ', PMR>=0.5: {:.3f}'.format(
        coarse_matching_meter.mean('PMR>=0.5'))
    ...
    message += ', median_RRE: {:.3f}'.format(
        registration_meter.mean('median_rre'))
    message += ', median_RTE: {:.3f}'.format(
        registration_meter.mean('median_rte'))
    logger.critical(message)
    ...
    # 3. print average metric on all scan pairs
    message = '  All Scan Pairs'
    message += ', RR: {:.3f}'.format(all_scan_pair_meter.mean('recall'))
    message += ', FMR: {:.3f}'.format(fine_matching_meter.mean('recall'))
    message += ', IR: {:.3f}'.format(fine_matching_meter.mean('inlier_ratio'))
    message += ', RRE: {:.3f}'.format(all_scan_pair_meter.mean('rre'))
    message += ', RTE: {:.3f}'.format(all_scan_pair_meter.mean('rte'))
    logger.critical(message)
...
def main():
    ...
    if args.method == 'lgr' or args.method == 'svd':
        log_file = osp.join(
            cfg.model_dir,
            'eval-{}-{}-{}.log'.format(args.benchmark, args.method,
