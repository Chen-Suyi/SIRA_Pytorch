...
from common.utils.threedmatch import get_scene_abbr
...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_root in scene_roots:
        ...
        for file_name in file_names:
            ...
            rre, rte = compute_registration_error(transform,
                                                  estimated_transform)
    ...
    message += ', RTE: {:.4f}'.format(registration_meter.mean('overall_rte'))
    message += ', mean_RR: {:.3f}'.format(registration_meter.mean('recall'))
    message += ', mean_RRE: {:.3f}'.format(registration_meter.mean('rre'))
    message += ', mean_RTE: {:.4f}, '.format(registration_meter.mean('rte'))
    logger.critical(message)
    for scene_abbr, result_dict in scene_registration_result_dict.items():
        ...
        message += ', RTE: {:.4f}'.format(result_dict['rte'])
        logger.critical(message)
...
def main():
    ...
    if args.method == 'lgr' or args.method == 'svd':
        log_file = osp.join(
            cfg.model_dir,
            'eval_dgr-{}-{}-{}.log'.format(args.benchmark, args.method,
