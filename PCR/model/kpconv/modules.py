...
from model.kpconv.functional import maxpool, nearest_upsample, global_avgpool, knn_interpolate
...
class KNNInterpolate(nn.Module):
    ...
    def forward(self, s_feats, q_points, s_points, neighbor_indices):
        if self.k == 1:
            ...
            return knn_interpolate(s_feats,
                                   q_points,
                                   s_points,
                                   neighbor_indices,
                                   self.k,
...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.KPConv = KPConv(in_channels,
                             out_channels,
                             kernel_size,
                             radius,
                             sigma,
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        Args:
            in_channels: dimension input features
            out_channels: dimension output features
            kernel_size: number of kernel points
            radius: convolution radius
            sigma: influence radius of each kernel point
            group_norm: group number for GroupNorm
        ...
        if in_channels != mid_channels:
            self.unary1 = UnaryBlock(in_channels,
                                     mid_channels,
                                     group_norm,
                                     bias=bias,
        ...
        self.KPConv = KPConv(mid_channels,
                             mid_channels,
                             kernel_size,
                             radius,
                             sigma,
        ...
        self.unary2 = UnaryBlock(mid_channels,
                                 out_channels,
                                 group_norm,
                                 has_relu=False,
                                 bias=bias,
        ...
        if in_channels != out_channels:
            self.unary_shortcut = UnaryBlock(in_channels,
                                             out_channels,
                                             group_norm,
                                             has_relu=False,
                                             bias=bias,
