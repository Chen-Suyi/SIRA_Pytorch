...
class VanillaConditionalTransformer(nn.Module):
    ...
    def forward(self, feats0, feats1, masks0=None, masks1=None):
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0, scores0 = self.layers[i](feats0,
                                                 feats0,
                                                 memory_masks=masks0)
                feats1, scores1 = self.layers[i](feats1,
                                                 feats1,
                                                 memory_masks=masks1)
            else:
                feats0, scores0 = self.layers[i](feats0,
    ...
    def forward(self,
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                ...
                if self.parallel:
                    new_feats0, scores0 = self.layers[i](feats0,
                                                         feats1,
                                                         memory_masks=masks1)
                    new_feats1, scores1 = self.layers[i](feats1,
                                                         feats0,
                                                         memory_masks=masks0)
                    feats0 = new_feats0
                    feats1 = new_feats1
