...
class LRPEMultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, num_heads, num_embeddings, dropout=None):
        ...
        self.embedding = LearnablePositionalEmbedding(num_embeddings,
                                                      d_model,
                                                      dropout=dropout)
    ...
    def forward(
        ...
        """
        q = rearrange(self.proj_q(input_q),
                      'b n (h c) -> b h n c',
                      h=self.num_heads)
        k = rearrange(self.proj_k(input_k),
                      'b m (h c) -> b h m c',
                      h=self.num_heads)
        v = rearrange(self.proj_v(input_v),
                      'b m (h c) -> b h m c',
                      h=self.num_heads)
        ...
        attention_scores = (attention_scores_e +
                            attention_scores_p) / self.d_model_per_head**0.5
        if attention_factors is not None:
            attention_scores = attention_factors.unsqueeze(
                1) * attention_scores
...
class LRPEAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model, num_heads, rpe_size, dropout=None):
        ...
        self.attention = LRPEMultiHeadAttention(d_model,
                                                num_heads,
                                                rpe_size,
                                                dropout=dropout)
...
class LRPETransformerLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attention = LRPEAttentionLayer(d_model,
                                            num_heads,
                                            rpe_size,
                                            dropout=dropout)
