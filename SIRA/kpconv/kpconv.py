...
from kpconv.kernel_points import load_kernels
...
def index_select(data: torch.Tensor, index: torch.LongTensor,
    ...
    """
    output = data.index_select(dim, index.contiguous().view(-1))
...
class KPConv(nn.Module):
    ...
    def initialize_kernel_points(self):
        ...
        kernel_points = load_kernels(self.radius,
                                     self.kernel_size,
    ...
    def forward(self, s_feats, q_points, s_points, neighbor_indices):
        ...
        """
        s_points = torch.cat(
            [s_points, torch.zeros_like(s_points[:1, :]) + self.inf],
            0)  # (N, 3) -> (N+1, 3)
        neighbors = index_select(s_points, neighbor_indices,
                                 dim=0)  # (N+1, 3) -> (M, H, 3)
        ...
        neighbor_weights = torch.clamp(1 -
                                       torch.sqrt(sq_distances) / self.sigma,
                                       min=0.0)  # (M, H, K)
        neighbor_weights = torch.transpose(neighbor_weights, 1,
                                           2)  # (M, H, K) -> (M, K, H)

        # apply neighbor weights
        s_feats = torch.cat((s_feats, torch.zeros_like(s_feats[:1, :])),
                            0)  # (N, C) -> (N+1, C)
        neighbor_feats = index_select(s_feats, neighbor_indices,
                                      dim=0)  # (N+1, C) -> (M, H, C)
        weighted_feats = torch.matmul(
            neighbor_weights,
            neighbor_feats)  # (M, K, H) x (M, H, C) -> (M, K, C)

        # apply convolutional weights
        weighted_feats = weighted_feats.permute(1, 0,
                                                2)  # (M, K, C) -> (K, M, C)
        kernel_outputs = torch.matmul(
            weighted_feats,
            self.weights)  # (K, M, C) x (K, C, C_out) -> (K, M, C_out)
        output_feats = torch.sum(kernel_outputs, dim=0,
                                 keepdim=False)  # (K, M, C_out) -> (M, C_out)
